...
static u8 bgt928 = FALSE; // 新增GT928驱动
...
s32 gtp_send_cfg(struct i2c_client *client, int len);
...
s32 gtp_send_cfg(struct i2c_client *client, int len)
{
    ...
    if (len > (GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH)) {
        len = GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH;
    }
    for (retry = 0; retry < 5; retry++) {
        ret = gtp_i2c_write(client, config, len);
        if (ret > 0) {
            break;
        }
    }
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if ((finger == 0x00) && (CHIP_TYPE_GT9F == ts->chip_type)) // request arrived
    {
        ...
        case GTP_RQST_CONFIG:
            ...
            ret = gtp_send_cfg(ts->client, (GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH));
            if (ret < 0) {
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    // 新增GT928驱动
    if (bgt928) {
        send_cfg_buf[0] = gtp_dat_gt28;
        cfg_info_len[0] = CFG_GROUP_LEN(gtp_dat_gt28);
    }
...
static ssize_t gt91xx_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    ...
    // printk("copy_from_user, count = %d\n", count);

    ret = gtp_send_cfg(i2c_connect_client, count);
...
s32 gtp_dump_reg_by_become(struct i2c_client *client)
{
    u8 buf[200];
    s32 ret;
    int i;

    buf[0] = (0xff00 & GOODIX_REG_CONFIG_DATA) >> 8;
    buf[1] = 0xff & GOODIX_REG_CONFIG_DATA;
    ret = gtp_i2c_read(client, buf, sizeof(buf));
    if (ret < 0) {
        GTP_ERROR("Gtp_dump_reg failed");
        return ret;
    }

    printk("Gtp dump hex, from 0x8047:");
    for (i = 0; i < (sizeof(buf) - 2); i++) {
        if (i % 16 == 0) {
            printk("\n");
        }
        printk("0x%02x,", buf[i + 2]);
    }
...
#endif
    {
        gtp_send_cfg(ts->client, (GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH));
    }
...
/*
dump from rk3288 mipi screen gt9xx touchscreen: 1200*1920:
version:0x63,       //'c'
Xsize:0xb0,0x04,    //0x4b0 = 1200
Ysize:0x80,0x07,    //0x780 = 1920
0x0a,               //touch number
,
